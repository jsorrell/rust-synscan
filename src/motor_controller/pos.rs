...
impl<T: SerialPort> MotorController<T> {
    ...
    pub fn inquire_pos(&self, channel: SingleChannel) -> SynScanResult<i32> {
    ...
    pub fn inquire_pos_degrees(&self, channel: SingleChannel) -> SynScanResult<f64> {
    ...
    pub fn set_pos(&self, channel: impl Channel, counts: i32) -> SynScanResult<()> {
        // Data is offset by 0x800000 according to spec
        self.port
    ...
    /// Only changes encoder number -- doesn't move mount.
    pub fn set_pos_degrees(&self, channel: SingleChannel, degrees: f64) -> SynScanResult<()> {
        let counts = self
            .motor_parameters
