mod comm {
    ...
    impl <'a> MotorControllerPort<'a> {
        pub(crate) fn bytes_to_number(data: Vec<u8>) -> SynScanResult<u32> {
            ...
            match u32::from_str_radix(&hex_string, 16) {
                Ok(v) => Ok(v),
                Err(e) => Err(SynScanError::CommunicationError(io::Error::new(ErrorKind::InvalidData, e)))
            }
        }
        ...
        fn check_amount_read(&self, size: usize) -> io::Result<()> {
            if size != 0 { Ok(()) } else { Err(io::Error::from(TimedOut)) }
        }
        ...
        fn read_response(&mut self) -> SynScanResult<Vec<u8>> {
            ...
            return if byte == b'=' {
                ...
                loop {
                    ...
                    if byte == b'\r' {
                        ...
                        if i < MAX_VALID_RESPONSE {
                            ...
                            return Err(SynScanError::CommunicationError(io::Error::from(InvalidData)));
                        }
                    }
                };
                ...
                Err(SynScanError::CommunicationError(io::Error::from(InvalidData)))
            }
        }

        fn raw_send_cmd(&mut self, cmd: u8, channel: &dyn Channel, bytes: &[u8]) -> SynScanResult<Vec<u8>> {
        ...
        pub fn send_cmd_bytes(&mut self, cmd: u8, channel: &dyn Channel, bytes: &[u8]) -> SynScanResult<()> {
            self.raw_send_cmd(cmd, channel, bytes)?;
            Ok(())
        }

        pub fn send_cmd_number(&mut self, cmd: u8, channel: &dyn Channel, number: u32, num_bytes: usize) -> SynScanResult<()> {
            self.send_cmd_bytes(cmd, channel, Self::number_to_bytes(number, num_bytes).as_slice())
        }
    ...
    mod tests {
        ...
        fn test_get_number_from_response() {
            assert_eq!(MotorControllerPort::bytes_to_number(vec![b'3', b'1']).unwrap(), 0x31);
            assert_eq!(MotorControllerPort::bytes_to_number(vec![b'3', b'1', b'7', b'F']).unwrap(), 0x7f31);
            assert_eq!(MotorControllerPort::bytes_to_number(vec![b'7', b'2', b'9', b'D', b'B', b'E']).unwrap(), 0xBE9D72);
            assert_eq!(MotorControllerPort::bytes_to_number(vec![b'5', b'6', b'3', b'4', b'1', b'2']).unwrap(), 0x123456);
        }
        #[test]
        fn test_get_data_from_number() {
            assert_eq!(MotorControllerPort::number_to_bytes(22, 2), vec![b'1', b'6']);
            assert_eq!(MotorControllerPort::number_to_bytes(22, 4), vec![b'1', b'6', b'0', b'0']);
            assert_eq!(MotorControllerPort::number_to_bytes(22, 6), vec![b'1', b'6', b'0', b'0', b'0', b'0']);
            assert_eq!(MotorControllerPort::number_to_bytes(123456, 6), vec![b'4', b'0', b'E', b'2', b'0', b'1']);
        }
    }
}
